...
% Last Modified by GUIDE v2.5 05-Mar-2023 22:38:24
...
for i=1:index
    ...
    count=0;
    if imdata.maskon==0
       imdata.filtro.FIL{1,i}=ones(l,c);
    end
    for i1=1:l
        for i2=1:c
...
T=table(transpose(imdata.filename), transpose(imdata.STATS.std), transpose(imdata.STATS.zim), transpose(imdata.ParC.LCO),'VariableNames',{'file','std','zim','grey'});

outname2=[respath '\' 'stats_output.xls'];
